...
Suppose you have a component that uses `setInterval` to periodically update state. When this component unmounts then, you need to `clearInterval` because you can't have the interval continuing to try to update state of an _unmounted_ component. In fact, React will throw an error if you do.
